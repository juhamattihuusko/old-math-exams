...
\newpage
\item Määritä kuvaan varjostettu pinta-ala sekä kuvassa olevan suoran kulmakerroin.

\begin{tikzpicture}[>=stealth]
\path[clip] (-5,-5) rectangle (6,10);
\filldraw[color=red!50!white, smooth, domain=-2:0]   plot (\x,{2^(\x)}) -- (0,0)--(-2,0)--cycle;
\draw[gray] (-8.9,-8.9) grid (5,9);
\draw[->,thick] (-10,0) -- (5,0) node[below]{$x$};
\draw[->,thick] (0,-10) -- (0,9) node[left]{$y$};
\filldraw (2,4) circle (2pt);


\draw[thick,color=blue, smooth, domain=-3:3]   plot (\x,{2^(\x)})    node[right] {$f(x) = 2^x$};
\draw (2+1,4+4*0.69) -- (2-3,4-4*3*0.69);
\end{tikzpicture}
